...
def LDPC_test_single_trial_run(H, G_t, bit_err_prob, max_num_iterations):
    ...
    if(decoded_cw[1]):
        ...
        err_count = 1
    return(bit_err_prob,err_count,undet_err_count,decoded_cw[2],decoded_cw[1],LLRs,cw)
...
def run(H, LLR_init, max_iterations):
    ...
    for i in range(max_iterations):
        ...
        #print('ck6')
##        print (np.dot(H,LLR_to_cw(LLRs))%2).T
##        print "Ham_dist = %d"%np.sum(np.dot(H,LLR_to_cw(LLRs))%2)
...
if __name__ == '__main__':
    H = parse_MacKay_dense('sim.txt',102,204)
    G_t = np.loadtxt('simm.txt')
    #H = parse_MacKay_dense('204.33.484 (N=204,K=102,M=102,R=0.5).txt',102,204)
    #G_t = np.loadtxt('204.33.484 (N=204,K=102,M=102,R=0.5)G_t.txt')
    p = Pool(4)
    t = clock()
    #result = p.map(LDPC_test_AWGN_run, [(H,G_t,0.01*i,10,100) for i in range(30,3,-1)])
    result = p.map(LDPC_test_run, [(H,G_t,0.01*i,30,1000) for i in range(10,1,-1)])
    t_post = clock()-t
...
########################################################
## Script for testing the C implementation of NB-LDPC
########################################################
##H = parse_MacKay_dense('204.33.484 (N=204,K=102,M=102,R=0.5).txt',102,204)
##G_t = np.loadtxt('204.33.484 (N=204,K=102,M=102,R=0.5)G_t.txt')
...
##while (undet_er_c == 0):
##    res_prev = res
##    res = LDPC_test_single_trial_run(H, G_t, 0.05, 10)
##    er_c = res[1]
##    undet_er_c = res[2]
##
##for i in range(204):
##    print 'x[%d] = %d;'%(i,res[6][i])
##
##for i in range(204):
##    print 'q0[%d] = %f;'%(i,res[5][i])
